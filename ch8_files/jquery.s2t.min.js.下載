...
! function($) {
	...
	function tranStr(str, toT) {
		var i, letter, code, isChinese, index, src, des, result = "";
		if (toT ? (src = S, des = T) : (src = T, des = S), "string" != typeof str) return str;
		for (i = 0; i < str.length; i++) letter = str.charAt(i), (isChinese = (code = str.charCodeAt(i)) > 13312 && code <
				40899 || code > 63744 && code < 64106) ? result += -1 !== (index = src.indexOf(letter)) ? des.charAt(index) :
	...
	function tranAttr(element, attr, toT) {
		...
		if (attr instanceof Array)
			for (i = 0; i < attr.length; i++) tranAttr(element, attr[i], toT);
		else "" !== (attrValue = element.getAttribute(attr)) && null !== attrValue && element.setAttribute(attr, tranStr(
			attrValue, toT))
	}
	...
	function tranElement(element, toT) {
		...
		if (1 === element.nodeType)
			for (childNodes = element.childNodes, i = 0; i < childNodes.length; i++) {
				var childNode = childNodes.item(i);
				if (1 === childNode.nodeType) {
					if (-1 !== "|BR|HR|TEXTAREA|SCRIPT|OBJECT|EMBED|".indexOf("|" + childNode.tagName + "|")) continue;
					tranAttr(childNode, ["title", "data-original-title", "alt", "placeholder"], toT), "INPUT" === childNode.tagName &&
						"" !== childNode.value && "text" !== childNode.type && "hidden" !== childNode.type && (childNode.value = tranStr(
							childNode.value, toT)), tranElement(childNode, toT)
				} else 3 === childNode.nodeType && (childNode.data = tranStr(childNode.data, toT))
			}
	}
	$.extend({
		s2t: function(str) {
			return tranStr(str, !0)
		},
		t2s: function(str) {
			return tranStr(str, !1)
		}
	}), $.fn.extend({
		s2t: function() {
